...
public class ResultResponseBodyAdvice implements ResponseBodyAdvice<Object> {
  ...
  private boolean hasIgnoreWrapper(MethodParameter returnType) {
    Class<?> containingClass = Objects.requireNonNull(returnType.getContainingClass());
    return AnnotatedElementUtils.hasAnnotation(containingClass, IgnoreResultWrapper.class)
        || (returnType.getMethod() != null
            && AnnotatedElementUtils.hasAnnotation(
                returnType.getMethod(), IgnoreResultWrapper.class));
  }

  private boolean isWrapperEligible(Class<?> rawType) {
    return !void.class.isAssignableFrom(rawType)
        && !Result.class.isAssignableFrom(rawType)
        && !HttpEntity.class.isAssignableFrom(rawType)
        && !Resource.class.isAssignableFrom(rawType)
